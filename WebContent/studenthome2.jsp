...
.button5 {background-color: #555555;} /* Black */
	...
	<%
      List<Testinfo> list1=DBConnnection.gettestdata();


	List<Testinfo> list=new ArrayList<Testinfo>();
	 for(int i=0;i<list1.size();i++)
	 {
		 Testinfo t=list1.get(i);
	
		 int count=DBConnnection.getTestQuestionCount(t.getTestid());
		// int t_id	= t.getTestid();
		 if(count>=17)
		 {
		 list.add(t);	 
		 }
    ...
    %>
        ...
        <tbody>
            <tr style="background-color:background-color: #20b2aa66;">
            
			<% 
			for(int i=0;i<list.size();i++)
			{
				Testinfo info=list.get(i);
	//int t_id=info.getTestid();
	
			%>	
	
			         <td align="center"><span><%=i+1%></span></td>
			         <td align="center"><span><%=info.getTname()%></span></td>
			         <td align="center"><span><%=info.getCtime()%></span></td>
			         <td align="center"><span><%=info.getCdate()%></span></td>
			         

					
					<form action="InfoController" method="post"> 
					<td align="center">
					<button style="width: 88px;height: 36px; background-color: bisque; border-radius: 10px;" type="submit" name="ViewGraph" value="<%=info.getTestid()%>">StartTest</button></td>
